            ...
            for (i=0; i<info.length; i++) {
                if (info[i].type === 'extension') {
                    ...
                    if (hidden.indexOf(info[i].id) === -1) {
                        ...
                        let options = info[i].optionsUrl ? `<a class="options" href="${info[i].optionsUrl}" target="_blank" >Options</a>` : "";
                        let homepage = info[i].homepageUrl ? `<a class="homepage" href="${info[i].homepageUrl}" target="_blank" >Homepage</a>` : "";
                        let install = info[i].installType!=="normal" ? `(${info[i].installType})` : "";
                        if(info[i].updateUrl == null){
                            install += " (manual)";
                        }
                        let toggleDisabled = info[i].id===thisID ? "disabled" : "";
                        extItem.innerHTML = `
                            <div class="header">
                                <img class="icon" src="${icon}" />
                                <div class="name">${info[i].name}</div>
                                <input type="checkbox" tabindex="-1" ${checked} ${toggleDisabled}></input>
                            </div>
                            <div class="details">
